...
# 商品列表原链接：https://gatewx.dmall.com/customersite/searchWareByCategory?param=%7B%22pageNum%22%3A1%2C%22pageSize%22%3A30%2C%22venderId%22%3A%221%22%2C%22storeId%22%3A%22108%22%2C%22sort%22%3A%221%22%2C%22categoryId%22%3A11215%2C%22categoryLevel%22%3A3%2C%22cateSource%22%3A1%2C%22bizType%22%3A%221%22%7D&token=&source=2&tempid=C7C148906AA000022AB812DF113D9500&pubParam=%7B%22utmSource%22%3A%22wxmp%22%7D&_=1512312503345
# 商品列表url解码后连接：https://gatewx.dmall.com/customersite/searchWareByCategory?param={"pageNum":1,"pageSize":30,"venderId":"1","storeId":"108","sort":"1","categoryId":11215,"categoryLevel":3,"cateSource":1,"bizType":"1"}&token=&source=2&tempid=C7C148906AA000022AB812DF113D9500&pubParam={"utmSource":"wxmp"}&_=1512312503345

# https://i.dmall.com/?source_id=wxmp#item/view/item/item:id=1-108-10735
...
class PythonIceCream():
    def __init__(self):
        # 爬取分类修改修改：商品列表地址、商品详情地址、存储的商品分类id=10901
        self.url =  'https://gatewx.dmall.com/customersite/searchWareByCategory?param={"pageNum":2,"pageSize":17,"venderId":"1","storeId":"108","sort":"1","categoryId":11215,"categoryLevel":3,"cateSource":1,"bizType":"1"}&token=&source=2&tempid=C7C148906AA000022AB812DF113D9500&pubParam={"utmSource":"wxmp"}&_=1512312503345'
    ...
    def get_connect(self):
        ...
        try:
            # 创建表
            self.cur.execute(
                'CREATE TABLE ' + self.tablename + ' (id BIGINT(7) NOT NULL AUTO_INCREMENT, title VARCHAR(1000), img VARCHAR(1000), lunfanimg VARCHAR(1000), spec VARCHAR(1000), xcimg VARCHAR(2000), skuid VARCHAR(1000),pimg VARCHAR(1000), plunfanimg VARCHAR(1000),pxcimg VARCHAR(2000) ,categoryid VARCHAR(1000),price VARCHAR(1000),brandname VARCHAR(1000),categoryname VARCHAR(1000),created TIMESTAMP DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY(id))')
        ...
        self.cur.execute(
            'ALTER TABLE ' + self.tablename + ' CHANGE xcimg xcimg VARCHAR(2000) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci')
        self.cur.execute(
        ...
        self.cur.execute(
            'ALTER TABLE ' + self.tablename + ' CHANGE pxcimg pxcimg VARCHAR(2000) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci')
        self.cur.execute(
        ...
        for item in listdata:
           ...
           # 判断数据库是否有skuId，有就不插入，无则插入
            ...
            if result:
                ...
                (itemdir, self.dataurl, json.dumps(lunfantu1,ensure_ascii=False), json.dumps(specpagram,ensure_ascii=False), json.dumps(xuanchuan1,ensure_ascii=False),str(item[3]),self.pimg,json.dumps(plunfantu,ensure_ascii=False),json.dumps(pxuanchuan,ensure_ascii=False),'11215','%.2f'%(item[4]/100),str(item[5]),str(item[6])))
...
if __name__ == "__main__":
    ...
    # for page in range(1,3):
        pythonIceCream = PythonIceCream()
        pythonIceCream.get_mkdir()
        print(" -- 爬取完毕 -- ")
